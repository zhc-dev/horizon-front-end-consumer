...
import { getContestListService, enterContestService } from '@/apis/contest'
...
async function enterExam(contestId) {
  await checkLogin()
  if (!isLogin.value) {
    ElMessage.error('请先登录后报名参赛,谢谢')
    return
  }
  const enterContestRequest = ref({
    contestId: contestId
  })
  await enterContestService(enterContestRequest.value)
...
function goContest(contest) {
  ...
  router.push(`/coding?contestId=${contest.contestId}&contestTitle=${contest.title}&contestEndTime=${contest.endTime}`)
}
